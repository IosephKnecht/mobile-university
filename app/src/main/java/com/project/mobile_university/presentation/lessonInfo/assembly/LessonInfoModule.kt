...
import com.project.mobile_university.presentation.lessonInfo.interactor.LessonInfoInteractor
...
class LessonInfoModule {
    ...
    fun provideInteractor(scheduleRepository: ScheduleRepository): LessonInfoContract.Interactor {
        return LessonInfoInteractor(scheduleRepository)
    }
}

@PerFeatureLayerScope
class LessonInfoFactory @Inject constructor(private val interactor: LessonInfoContract.Interactor,
                                            private val lessonId: Long) : ViewModelProvider.Factory {
    @Suppress("UNCHECKED_CAST")
    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        return LessonInfoPresenter(lessonId, interactor) as T
    }
